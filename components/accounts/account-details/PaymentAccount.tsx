...
import { TransactionStatus, transactionsData } from "../bank-account/PaymentAccount";

const sortOptions = ["Recent", "Name", "Amount"];

const PaymentAccount = () => {
  const [selected, setSelected] = useState(sortOptions[0]);
  const { search, sortData, currentPage, deleteItem, totalData, endIndex, nextPage, paginate, prevPage, startIndex, tableData, totalPages } = useTable(transactionsData);
        ...
        <table className="w-full whitespace-nowrap">
          <thead>
            <tr className="bg-secondary/5 dark:bg-bg3">
              ...
              <th onClick={() => sortData("currency")} className="text-start py-5 min-w-[120px] cursor-pointer">
                <div className="flex items-center gap-1">
              ...
              <th onClick={() => sortData("status")} className="text-start py-5 cursor-pointer min-w-[120px]">
                <div className="flex items-center gap-1">
